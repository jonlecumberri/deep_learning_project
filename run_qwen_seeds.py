...
for seed in SEEDS:
    ...
    def compute_metrics(pred):
        ...
        report = classification_report(labels, preds, target_names=["NoHate", "Hate"], output_dict=True, zero_division=0)
        return {"eval_accuracy": acc, "eval_f1_Hate": report["Hate"]["f1-score"], "eval_f1_NoHate": report["NoHate"]["f1-score"]}
    ...
    gold_labels = eval_df["label"].tolist()
    print(f"\nSample predictions for SEED {seed}:")
    for i in range(5):
        text_preview = eval_texts[i][:100] + ('...' if len(eval_texts[i]) > 100 else '')
        gold = "Hate" if gold_labels[i] == 1 else "NoHate"
        pred = "Hate" if pred_labels[i] == 1 else "NoHate"
    ...
    pd.DataFrame({"step": list(range(len(all_train_loss))), "loss": all_train_loss}).to_csv(
        os.path.join(RESULTS_DIR, "qwen_train_loss.csv"), index=False
    )
