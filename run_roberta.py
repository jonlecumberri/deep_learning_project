...
RESULTS_DIR = os.path.join(BASE_DIR, "DeepLearning/project/results", "results_roberta_offensive")
MODEL_DIR = os.path.join(BASE_DIR, "DeepLearning/project/models", "roberta_offensive_model")
...
print("🧠 Loading RoBERTa offensive model...")
model = AutoModelForSequenceClassification.from_pretrained(
    "cardiffnlp/twitter-roberta-base-offensive",
    num_labels=2,
    device_map="auto",
    use_safetensors=True  # 👈 force use of the secure format
)
...
bar_metrics_path = os.path.join(RESULTS_DIR, "roberta_finetuned_barplot_data.csv")
pd.DataFrame([
...
plt.ylim(0, 1)
plt.ylabel("Score")
plt.title("RoBERTa Final Evaluation Metrics")
plt.savefig(os.path.join(RESULTS_DIR, "roberta_finetuned_barplot.png"))
plt.close()
...
progress_path = os.path.join(RESULTS_DIR, "roberta_train_progress_data.csv")
pd.DataFrame({
...
plt.xlabel("Step")
plt.title("RoBERTa Training Checkpoints")
plt.legend()
plt.savefig(os.path.join(RESULTS_DIR, "roberta_train_progress.png"))
plt.close()
